...
from client_service.price_service import Price_service
...
class Macd_sprtrnd_strategy_2h:
    ...
    def trade_strategy(self):
        ...
        round_price = config.round_price
        ...
        while True:
            try:
                ...
                if self.future_action.check_open_position_bool(binance_client, config.ts_macd_symbol):
                    ...
                    self.control_close_position_thread = threading.Thread(
                        target=self.control_close_position(client=binance_client, indicators=indicators))

                    if not self.control_close_position_thread.is_alive():
                        self.control_close_position_thread.start()


                    self.control_position_thread = threading.Thread(
                        target=self.control_position.control_position_profit(binance_client, trading_symbol,
                                                                             trading_amount,
                                                                             round_price, round_amount))
                    if not self.control_position_thread.is_alive():
                        self.control_position_thread.start()

                    # control trailing stop

                    self.control_trailing_thread = threading.Thread(
                        target=self.control_position.control_trailing_callback(
                                                                               client=binance_client,
                                                                               new_callBack_rate=2.5,
                                                                               trading_symbol=trading_symbol,
                                                                               ))
                    if not self.control_trailing_thread.is_alive():
                        self.control_trailing_thread.start()

                    self.control_trailing_thread.join()
                    self.control_position_thread.join()

                    self.control_close_position_thread.join()
    ...
    def long_macd_position(self, client):
        ...
        print(f'LONG Trend {config.ts_macd_symbol}, current price -  {self.current_price} $')
        print('!!!!!!')
    ...
    def control_close_position(self, **kwargs):
        ...
        print(f"Control position {trading_symbol} {formatted_current_time} current_price - {current_price}"
              f" |  Position profit - {round(unrealized_profit, round_price)}\n")
        with open('macd_sptrndl_log.txt', 'a') as f:
            f.write(f"Control position {trading_symbol} {formatted_current_time}")
            f.write(
                f"macd1 - {macd_line_1} signal1 - {signal_line_1} | macd2 - {macd_line_2} signal2 - {signal_line_2}"
